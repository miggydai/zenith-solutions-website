...
const Grid: React.FC<Props> = ({ gotoSlide }) => {
  return (
    ...
    <div className="w-full sm:h-[80vh] h-screen flex justify-center items-center">
      <motion.div
        variants={gridContainerVariants}
        initial="hidden"
        animate="show"
        className="sm:w-[80%] w-full h-full flex sm:flex-row flex-col gap-2"
      >
        {/* first div */}
        <div className="sm:w-[60%] w-full sm:h-full h-[20vh] flex sm:flex-col flex-row gap-2">
          {/* Zeal Block */}
          <motion.div
            variants={gridVariants}
            whileHover={{ scale: 1.2 }}
            whileTap={{ scale: 0.9 }}
            className="relative group w-full h-full cursor-pointer group bg-tanSgradient rounded-2xl flex text-black flex-col justify-between p-5 font-bold transition ease-out duration-600"
            onClick={() => gotoSlide(0)}
          >
            <p className="font-montserrat sm:text-xl text-xs w-full">
              Project Ready
            </p>

            <p className="sm:text-xl text-xs font-montserrat w-full">10k+</p>
          ...
          <motion.div
            ...
            whileHover={{ scale: 1.2 }}
            whileTap={{ scale: 0.9 }}
            className="relative group cursor-pointer w-full h-full flex items-center justify-center bg-skyBlue rounded-2xl"
            onClick={() => gotoSlide(1)}
          >
            <p className="font-bold sm:text-2xl text-xs font-montserrat uppercase tracking-wide">
              Diversity
            </p>
        ...
        <motion.div
          ...
          whileHover={{ scale: 1.2 }}
          whileTap={{ scale: 0.9 }}
          className="relative group cursor-pointer w-full sm:h-full h-[40vh]"
          onClick={() => gotoSlide(2)}
        >
          <Image
            src={imgsrc}
        ...
        {/* thrid div */}
        <div className="w-full sm:h-full h-[50vh]  flex flex-col gap-3">
          {/* 4th */}
          <motion.div
            variants={gridVariants}
            whileHover={{ scale: 1.2 }}
            whileTap={{ scale: 0.9 }}
            className="relative group sm:h-full h-[25vh] w-full cursor-pointer flex items-center justify-center bg-tanSgradient rounded-2xl"
            onClick={() => gotoSlide(3)}
          >
            <p className="font-bold text-lg text-ceny=ter uppercase tracking-wide text-black font-montserrat">
              Community Services
            </p>
            <div className="absolute inset-0 bg-[#59D4D4] flex items-center justify-center opacity-0 group-hover:opacity-100 transition ease-out duration-600 rounded-2xl">
              <p className="text-white sm:text-2xl text-sm font-montserrat tracking-wide uppercase font-bold">
          ...
          <div className="w-full h-full  flex flex-row gap-3 justify-between">
            ...
            <motion.div
              ...
              whileHover={{ scale: 1.2 }}
              whileTap={{ scale: 0.9 }}
              className="relative group w-[50%] h-full bg-skyBlue cursor-pointer rounded-2xl flex text-white flex-col justify-between p-5 font-bold"
              onClick={() => gotoSlide(4)}
            >
              <p className="font-montserrat text-lg">Years of Experience</p>

              <h6 className="font-montserrat text-md sm:text-xl ">12+ Years</h6>
            ...
            <motion.div
              ...
              whileHover={{ scale: 1.2 }}
              whileTap={{ scale: 0.9 }}
              className="relative group w-[50%] p-2 cursor-pointer bg-skyBlue flex justify-center items-center rounded-2xl"
              onClick={() => gotoSlide(5)}
            >
              <p className="font-bold sm:text-xl text-xs font-montserrat uppercase">
                Inclusivity
              </p>
